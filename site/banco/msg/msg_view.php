...
function input_msg() {
    ...
    if (isset($_SESSION["msg_data"]["msg"])) {
        ...
        <textarea name="msg" placeholder="Mensagem..." id="msg">'. $_SESSION["msg_data"]["msg"].'</textarea>
        <p id="result">0/700</p>';
    } else {
...
function check_erros_msg() {
    if (isset($_SESSION['erros_msg'])) {
        $erros = $_SESSION['erros_msg'];
        ...
        unset($_SESSION['erros_msg']);
    } else if (isset($_GET["send"]) && $_GET["send"] === "success") {
        echo '<br>';
        echo '<p class="form-success">Enviado com sucesso!</p>'; 
    }
}